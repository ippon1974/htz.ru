...
$result[^MAIN:pSQL.table{
	...
	FROM
		news, navigation
	WHERE
	    article_type_id = ^hParam.article_type_id.int(3) AND
		is_published = 1 AND
...
^if($tArticle.id eq $tArticle.last_id){
		   ...
		   <dd><a href="/news/show/3/$tArticle.id/"><img src="/upload/small/$tArticle.image" alt="$tArticle.title" /></a></dd>
...
^if(def $tArticle.title && ^tArticle.title.match[\^[[^^\^]]+\^]]){
	$result[^tArticle.title.match[\^[([^^\^]]+)\^]][g]{<a title="$tArticle.title" href="/news/show/3/$tArticle.id/">$match.1</a>}]
}{
    $result[<a title="$tArticle.title" href="/news/show/3/$tArticle.id/">$tArticle.title</a>]
		...
		<td>
			^year.menu{
				<strong>^if($year.key == $curr_year){
					...
					<a href="/news/show/year/$year.key/month/$calendar.month/">$year.key</a>
				}
				</strong><br />
			}
		</td>
						...
						^if(def $form:month && $m == $curr_month){
							...
							^if(!($now.year == $year.key && $m > $now.month) && !($year.key == $min_year && $m < $min_month)){
								^if($ym_hash.[^year.key.format[%04d]=^m.format[%02d]]){
									<a href="/news/show/year/$year.key/month/$m/">$dtf:[ri-locale].month.$m</a>
								}{
...
^if(!def $form:id){
	...
	$calendar[^getCalendar[$.iArticleTypeId(3)]]
