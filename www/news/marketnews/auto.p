...
^if($tArticle.id eq $tArticle.last_id){
		   ...
		   <dd><a title="$tArticle.title" href="/news/marketnews/2/$tArticle.id/"><img src="/upload/small/$tArticle.image" alt="$tArticle.title" /></a></dd>
...
^if(def $tArticle.title && ^tArticle.title.match[\^[[^^\^]]+\^]]){
	$result[^tArticle.title.match[\^[([^^\^]]+)\^]][g]{<a title="$tArticle.title" href="/news/marketnews/2/$tArticle.id/">$match.1</a>}]
}{
    $result[<a title="$tArticle.title" href="/news/marketnews/2/$tArticle.id/">$tArticle.title</a>]
		...
		<td>
			^year.menu{
				<strong>^if($year.key == $curr_year){
					...
					<a href="/news/marketnews/year/$year.key/month/$calendar.month/">$year.key</a>
				}
				</strong><br />
			}
		</td>
...
^if(!def $form:id){
	...
	^rem{ *** получаем информацию о календаре и выводим его *** }
	$tCalendar[^getCalendar[$.iArticleTypeId(2)]]
