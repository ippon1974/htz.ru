...
<meta name="description" content="^page_select[] | Новости компании СТРОЙПРОЕКТМОНТАЖ." />
...
$result[^MAIN:pSQL.table{
	...
	FROM
		news, navigation
	WHERE
		article_type_id = ^hParam.iArticleTypeId.int(1) AND
		is_published = 1 AND
		dt_published <= ^MAIN:pSQL.now[]
		^if(^hParam.id.int(0)){
			AND article_id = ^hParam.id.int(1)
		}
			AND parents_id = '3'
	ORDER BY
		dt DESC
...
^if($tArticle.id eq $tArticle.last_id){
		   ...
		   <dd><a title="$tArticle.title" href="/news/cnews/1/$tArticle.id/"><img src="/upload/small/$tArticle.image" alt="$tArticle.title" /></a></dd>
...
^if(def $tArticle.title && ^tArticle.title.match[\^[[^^\^]]+\^]]){
	$result[^tArticle.title.match[\^[([^^\^]]+)\^]][g]{<a title="$tArticle.title" href="/news/cnews/1/$tArticle.id/">$match.1</a>}]
}{
    $result[<a title="$tArticle.title" href="/news/cnews/1/$tArticle.id/">$tArticle.title</a>]
...
@printArticleItem_call[]
^if($tArticle.id eq $tArticle.last_id){
$result[<img src="/upload/$tArticle.image" alt="$tArticle.title" />$tArticle.body]
}{
$result[$tArticle.body]
}
...
^if($calendar){
	...
	<table border="0" cellpadding="1" cellspacing="2" align="right">
	<tr valign="top">
		<td>
			^year.menu{
				<b>^if($year.key == $curr_year){
					...
					<a href="/news/cnews/?year=$year.key&month=$calendar.month">$year.key</a>
				}
				</b><br />
			}
		</td>
						...
						^if(def $form:month && $m == $curr_month){
							...
							^if(!($now.year == $year.key && $m > $now.month) && !($year.key == $min_year && $m < $min_month)){
								^if($ym_hash.[^year.key.format[%04d]=^m.format[%02d]]){
									<a href="/news/cnews/?year=$year.key&month=$m">$dtf:[ri-locale].month.$m</a>
								}{
