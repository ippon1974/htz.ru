...
$result[^MAIN:pSQL.table{
	...
	FROM
		news, navigation
	WHERE
		article_type_id = ^hParam.iArticleTypeId.int(4) AND
		is_published = 1 AND
...
^if($tArticle.id eq $tArticle.last_id){
		   ...
		   <dd><a title="$tArticle.title" href="/news/item/4/$tArticle.id/"><img src="/upload/small/$tArticle.image" alt="$tArticle.title" /></a></dd>
...
^if(def $tArticle.title && ^tArticle.title.match[\^[[^^\^]]+\^]]){
	$result[^tArticle.title.match[\^[([^^\^]]+)\^]][g]{<a title="$tArticle.title" href="/news/item/4/$tArticle.id/">$match.1</a>}]
}{
    $result[<a title="$tArticle.title" href="/news/item/4/$tArticle.id/">$tArticle.title</a>]
...
$result[^MAIN:pSQL.sql[table]{
	...
	FROM
		news
	WHERE
	    article_type_id = ^hParam.article_type_id.int(1) AND
		is_published = 1 AND
		dt_published <= ^MAIN:pSQL.now[]   		
	GROUP BY
		year DESC,
		...
		<td>
			^year.menu{
				<b>^if($year.key == $curr_year){
					...
					<a href="/test/?year=$year.key&month=$calendar.month">$year.key</a>
				}
				</b><br />
			}
		</td>
						...
						^if(def $form:month && $m == $curr_month){
							...
							^if(!($now.year == $year.key && $m > $now.month) && !($year.key == $min_year && $m < $min_month)){
								^if($ym_hash.[^year.key.format[%04d]=^m.format[%02d]]){
									<a href="/test/?year=$year.key&month=$m">$dtf:[ri-locale].month.$m</a>
								}{
