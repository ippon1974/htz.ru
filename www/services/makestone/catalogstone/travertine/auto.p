...
^opage.print[
	$.target_url[/services/makestone/catalogstone/travertine/]
	$.nav_count(25)
	$.mode[html]
]
$travertine[^MAIN:pSQL.table{
	SELECT
		travertine_slab_id AS id,
		travertine_slab_type_id,
		travertine_slab_type_id,
		travertine_name,
		travertine_desc,
		travertine_img,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND travertine_slab_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
