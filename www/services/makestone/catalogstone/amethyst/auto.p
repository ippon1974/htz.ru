...
^opage.print[
	$.target_url[/services/makestone/catalogstone/amethyst/]
	$.nav_count(25)
	$.mode[html]
]
$amethyst[^MAIN:pSQL.table{
	SELECT
		amethyst_slab_id AS id,
		amethyst_slab_type_id,
		amethyst_name,
		amethyst_desc,
		amethyst_img,
		is_published
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND amethyst_slab_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
