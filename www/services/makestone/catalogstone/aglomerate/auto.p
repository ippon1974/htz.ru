...
^opage.print[
	$.target_url[/services/makestone/catalogstone/aglomerate/]
	$.nav_count(25)
	$.mode[html]
]
$aglomerate[^MAIN:pSQL.table{
	SELECT
		aglomerate_slab_id AS id,
		aglomerate_slab_type_id,
		aglomerate_name,
		aglomerate_desc,
		aglomerate_img,
		is_published
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND aglomerate_slab_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
