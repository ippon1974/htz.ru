...
@title[]
$title[^db::getAglomerateSlab[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Слеб из агломерата $title.aglomerate_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getAglomerateSlab[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Слеб из $name $navi_center_id.aglomerate_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_slab_$item[^db::getAglomerateSlab[$.id(^form:id.int(0))]]
...
<div id="">
<h1>Слеб из $name ${detail_slab_$item.aglomerate_name}</h1>
</div>
...
<a href="/services/makestone/catalogstone/$item/img/big/${detail_slab_$item.aglomerate_img}.webp" target="_blank" title="Слеб из $name $detail_slab_$item.aglomerate_name"><img src="/services/makestone/catalogstone/$item/img/list/${detail_slab_$item.aglomerate_img}.webp" alt="Слеб из $name $detail_slab_$item.aglomerate_name" /></a>
...
<h3>Другие изделия из $name $detail_slab_$item.aglomerate_name</h3>
