...
^opage.print[
	$.target_url[/services/makestone/catalogstone/labradorite/]
	$.nav_count(25)
	$.mode[html]
]
$labradorite[^MAIN:pSQL.table{
	SELECT
		labradorite_slab_id AS id,
		labradorite_slab_type_id,
		labradorite_name,
		labradorite_desc,
		labradorite_img,
		is_published
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND labradorite_slab_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
