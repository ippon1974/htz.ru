...
^opage.print[
	$.target_url[/services/makestone/step/quartz/]
	$.nav_count(25)
	$.mode[html]
]
$quartz[^MAIN:pSQL.table{
	SELECT
		quartz_step_id AS id,
		quartz_slab_id,
		quartz_step_type_id,
		quartz_step_name,
		quartz_step_price,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND quartz_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.quartz_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.quartz_step_img}.webp" alt="Ступень из $name ${$item.quartz_step_name}" /></a></dt>
