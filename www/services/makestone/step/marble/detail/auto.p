...
@title[]
$title[^db::getMarbleStep[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Ступень из мрамора $title.marble_step_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getMarbleStep[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Ступень из $name $navi_center_id.marble_step_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_step_$item[^db::getMarbleStep[$.id(^form:id.int(0))]]
...
<div id="">
<h1>Ступень из $name ${detail_step_$item.marble_step_name}. Образец.</h1>
</div>
...
<a href="/services/makestone/step/$item/img/big/${detail_step_$item.marble_step_img}.webp" target="_blank" title="Ступень из $name $detail_step_$item.marble_step_name для кунхи"><img src="/services/makestone/step/$item/img/list/${detail_step_$item.marble_step_img}.webp" alt="Ступень из $name $detail_step_$item.marble_step_name для кунхи" /></a>
...
<h3>Другие изделия из $name $detail_step_$item.marble_step_name</h3>
