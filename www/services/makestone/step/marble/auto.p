...
^opage.print[
	$.target_url[/services/makestone/step/marble/]
	$.nav_count(50)
	$.mode[html]
]
$marble[^MAIN:pSQL.table{
	SELECT
		marble_step_id AS id,
		m_slab_id,
		marble_step_type_id,
		marble_step_name,
		marble_step_price,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND marble_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.marble_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.marble_step_img}.webp" alt="Ступень из $name ${$item.marble_step_name}" /></a></dt>
