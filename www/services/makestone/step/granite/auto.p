...
^opage.print[
	$.target_url[/services/makestone/step/granite/]
	$.nav_count(50)
	$.mode[html]
]
$granite[^MAIN:pSQL.table{
	SELECT
		granite_step_id AS id,
		g_slab_id,
		granite_step_type_id,
		granite_step_name,
		granite_step_price,
		granite_step_item,
		granite_step_desc,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND agat_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.granite_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.granite_step_img}.webp" alt="Ступень из $name ${$item.granite_step_name}" /></a></dt>
