...
^opage.print[
	$.target_url[/services/makestone/step/amethyst/]
	$.nav_count(25)
	$.mode[html]
]
$amethyst[^MAIN:pSQL.table{
	SELECT
		amethyst_step_id AS id,
		amethyst_slab_id,
		amethyst_step_type_id,
		amethyst_step_name,
		amethyst_step_price,
		amethyst_step_item,
		amethyst_step_desc,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND amethyst_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.amethyst_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.amethyst_step_img}.webp" alt="Ступень из $name ${$item.amethyst_step_name}" /></a></dt>
