...
$result[Ступени из ламбродарита]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/step/labradorite/]
	$.nav_count(25)
	$.mode[html]
]
$labradorite[^MAIN:pSQL.table{
	SELECT
		labradorite_step_id AS id,
		labradorite_slab_id,
		labradorite_step_type_id,
		labradorite_step_name,
		labradorite_step_price,
		labradorite_step_item,
		labradorite_step_desc,
		labradorite_step_img,
		is_published
	FROM
		v_step_labradorite
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND labradorite_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.labradorite_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.labradorite_step_img}.webp" alt="Ступень из $name ${$item.labradorite_step_name}" /></a></dt>
