...
^opage.print[
	$.target_url[/services/makestone/step/travertine/]
	$.nav_count(25)
	$.mode[html]
]
$travertine[^MAIN:pSQL.table{
	SELECT
		travertine_step_id AS id,
		travertine_slab_id,
		travertine_step_type_id,
		travertine_step_name,
		travertine_step_price,
		travertine_step_item,
		travertine_step_desc,
		travertine_step_img,
		is_published
	FROM
		v_step_travertine
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND travertine_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.travertine_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.travertine_step_img}.webp" alt="Ступень из $name ${$item.travertine_step_name}" /></a></dt>
