...
^opage.print[
	$.target_url[/services/makestone/step/aglomerate/]
	$.nav_count(25)
	$.mode[html]
]
$aglomerate[^MAIN:pSQL.table{
	SELECT
		aglomerate_step_id AS id,
		aglomerate_slab_id,
		aglomerate_step_type_id,
		aglomerate_step_name,
		aglomerate_step_price,
		aglomerate_step_item,
		aglomerate_step_desc,
		aglomerate_step_img,
		is_published
	FROM
		v_step_aglomerate
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND aglomerate_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.aglomerate_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.aglomerate_step_img}.webp" alt="Ступень из $name ${$item.aglomerate_step_name}" /></a></dt>
