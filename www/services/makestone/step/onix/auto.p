...
^opage.print[
	$.target_url[/services/makestone/step/onix/]
	$.nav_count(50)
	$.mode[html]
]
$onix[^MAIN:pSQL.table{
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND onix_step_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/step/$item/detail/id/${$item.id}/" title="Ступень из $name ${$item.onix_step_name}"><img src="/services/makestone/step/$item/img/list/${$item.onix_step_img}.webp" alt="Ступень из $name ${$item.onix_step_name}" /></a></dt>
