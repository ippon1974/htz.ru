...
$result[Столешницы из ламбродарита]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/tabletop/labradorite/]
	$.nav_count(0)
	$.mode[html]
]
$labradorite[^MAIN:pSQL.table{
	SELECT
		labradorite_tabletop_id AS id,
		labradorite_slab_id,
		labradorite_tabletop_type_id,
		labradorite_tabletop_name,
		labradorite_tabletop_price,
		labradorite_tabletop_item,
		labradorite_tabletop_desc,
		labradorite_tabletop_img,
		is_published
	FROM
		v_tabletop_labradorite
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND labradorite_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
