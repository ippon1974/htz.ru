...
^opage.print[
	$.target_url[/services/makestone/tabletop/quartz/]
	$.nav_count(25)
	$.mode[html]
]
$quartz[^MAIN:pSQL.table{
	SELECT
		quartz_tabletop_id AS id,
		quartz_sleb_id,
		quartz_tabletop_type_id,
		quartz_tabletop_name,
		quartz_tabletop_price,
		quartz_tabletop_item,
		quartz_tabletop_desc,
		quartz_tabletop_img,
		is_published
	FROM
		v_tabletop_quartz
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND onix_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
