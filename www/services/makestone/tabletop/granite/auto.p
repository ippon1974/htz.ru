...
^opage.print[
	$.target_url[/services/makestone/tabletop/granite/]
	$.nav_count(50)
	$.mode[html]
]
$granite[^MAIN:pSQL.table{
	SELECT
		granite_tabletop_id AS id,
		granite_slab_id,
		granite_tabletop_type_id,
		granite_tabletop_name,
		granite_tabletop_price,
		granite_tabletop_item,
		granite_tabletop_desc,
		granite_tabletop_img,
		is_published
	FROM
		v_tabletop_granite
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND granite_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
