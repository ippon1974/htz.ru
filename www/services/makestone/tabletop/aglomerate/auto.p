...
^opage.print[
	$.target_url[/services/makestone/tabletop/aglomerate/]
	$.nav_count(0)
	$.mode[html]
]
$aglomerate[^MAIN:pSQL.table{
	SELECT
		aglomerate_tabletop_id AS id,
		aglomerate_slab_id,
		aglomerate_tabletop_type_id,
		aglomerate_tabletop_name,
		aglomerate_tabletop_price,
		aglomerate_tabletop_item,
		aglomerate_tabletop_desc,
		aglomerate_tabletop_img,
		is_published
	FROM
		v_tabletop_aglomerate
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND aglomerate_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id DESC
}[
