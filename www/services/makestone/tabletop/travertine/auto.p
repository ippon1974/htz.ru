...
$result[Столешницы из травертина]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/tabletop/travertine/]
	$.nav_count(25)
	$.mode[html]
]
$travertine[^MAIN:pSQL.table{
	SELECT
		travertine_tabletop_id AS id,
		travertine_slab_id,
		travertine_tabletop_type_id,
		travertine_tabletop_name,
		travertine_tabletop_price,
		travertine_tabletop_item,
		travertine_tabletop_desc,
		travertine_tabletop_img,
		is_published
	FROM
		v_tabletop_travertine
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND travertine_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
