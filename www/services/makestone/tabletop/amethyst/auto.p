...
^opage.print[
	$.target_url[/services/makestone/tabletop/amethyst/]
	$.nav_count(0)
	$.mode[html]
]
$amethyst[^MAIN:pSQL.table{
	SELECT
		amethyst_tabletop_id AS id,
		amethyst_slab_id,
		amethyst_tabletop_type_id,
		amethyst_tabletop_name,
		amethyst_tabletop_price,
		amethyst_tabletop_item,
		amethyst_tabletop_desc,
		amethyst_tabletop_img,
		is_published
	FROM
		v_tabletop_amethyst
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND amethyst_tabletop_id = ^hParam.id.int(0)
		}
	ORDER BY
		id DESC
}[
