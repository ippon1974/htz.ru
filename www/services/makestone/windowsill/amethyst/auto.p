...
$result[Подоконники из аметиста]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/windowsill/amethyst/]
	$.nav_count(25)
	$.mode[html]
]
$amethyst[^MAIN:pSQL.table{
	SELECT
		amethyst_windowsill_id AS id,
		amethyst_slab_id,
		amethyst_windowsill_type_id,
		amethyst_windowsill_name,
		amethyst_windowsill_price,
		amethyst_windowsill_item,
		amethyst_windowsill_desc,
		amethyst_windowsill_img,
		is_published
	FROM
		v_windowsill_amethyst
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND amethyst_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
<ul class="listing">
	^amethyst.menu{
	       ^printList_cell[amethyst;аметиста]
	}
</ul>
}
#end @printList[opage]
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.amethyst_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.amethyst_windowsill_img}.webp" alt="Подоконник из $name ${$item.amethyst_windowsill_name}" /></a></dt>
