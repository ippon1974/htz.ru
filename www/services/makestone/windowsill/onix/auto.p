...
^opage.print[
	$.target_url[/services/makestone/windowsill/onix/]
	$.nav_count(50)
	$.mode[html]
]
$onix[^MAIN:pSQL.table{
	SELECT
		onix_windowsill_id AS id,
		onix_slab_id,
		onix_windowsill_type_id,
		onix_windowsill_name,
		onix_windowsill_price,
		onix_windowsill_item,
		onix_windowsill_desc,
		onix_windowsill_img,
		is_published
	FROM
		v_windowsill_onix
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND onix_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.onix_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.onix_windowsill_img}.webp" alt="Подоконник из $name ${$item.onix_windowsill_name}" /></a></dt>
