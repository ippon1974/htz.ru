...
$result[Подоконники из ламбродарита]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/windowsill/labradorite/]
	$.nav_count(25)
	$.mode[html]
]
$labradorite[^MAIN:pSQL.table{
	SELECT
		labradorite_windowsill_id AS id,
		labradorite_slab_id,
		labradorite_windowsill_type_id,
		labradorite_windowsill_name,
		labradorite_windowsill_price,
		labradorite_iwindowsill_tem,
		labradorite_windowsill_desc,
		labradorite_windowsill_img,
		is_published
	FROM
		v_windowsill_labradorite
	WHERE
...
<ul class="listing">
	^labradorite.menu{
	       ^printList_cell[labradorite;ламбродарита]
	}
</ul>
}
#end @printList[opage]
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.labradorite_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.labradorite_windowsill_img}.webp" alt="Подоконник из $name ${$item.labradorite_windowsill_name}" /></a></dt>
