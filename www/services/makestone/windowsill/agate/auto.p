...
^opage.print[
	$.target_url[/services/makestone/windowsill/agate/]
	$.nav_count(25)
	$.mode[html]
]
$agate[^MAIN:pSQL.table{
	SELECT
		agat_windowsill_id AS id,
		agat_slab_id,
		agat_windowsill_type_id,
		agat_windowsill_name,
		agat_windowsill_price,
		agat_windowsill_item,
		agat_windowsill_desc,
		agat_windowsill_img,
		is_published
	FROM
		v_windowsill_agate
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND agat_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.agat_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.agat_windowsill_img}.webp" alt="Подоконник из $name ${$item.agat_windowsill_name}" /></a></dt>
