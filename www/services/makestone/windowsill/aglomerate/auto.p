...
$result[Подоконники из агломерата]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/windowsill/aglomerate/]
	$.nav_count(25)
	$.mode[html]
]
$aglomerate[^MAIN:pSQL.table{
	SELECT
		aglomerate_windowsill_id AS id,
		aglomerate_slab_id,
		aglomerate_windowsill_type_id,
		aglomerate_windowsill_name,
		aglomerate_windowsill_price,
		aglomerate_windowsill_item,
		aglomerate_windowsill_desc,
		aglomerate_windowsill_img,
		is_published
	FROM
		v_windowsill_aglomerate
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND aglomerate_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
<ul class="listing">
	^aglomerate.menu{
	       ^printList_cell[aglomerate;агломерата]
	}
</ul>
}
#end @printList[opage]
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.aglomerate_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.aglomerate_windowsill_img}.webp" alt="Подоконник из $name ${$item.aglomerate_windowsill_name}" /></a></dt>
