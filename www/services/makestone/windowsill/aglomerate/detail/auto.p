...
@title[]
$title[^db::getAglomerateWindowsill[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Подоконник из агломерата $title.aglomerate_windowsill_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getAglomerateWindowsill[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Подоконник из $name $navi_center_id.aglomerate_windowsill_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_windowsill_$item[^db::getAglomerateWindowsill[$.id(^form:id.int(0))]]
$tabletop_$item[^db::getAglomerateTableTop[$.id(^form:id.int(0))]]
$step_$item[^db::getAglomerateStep[$.id(^form:id.int(0))]]
$table_$item[^db::getAglomerateTable[$.id(^form:id.int(0))]]
$slab_$item[^db::getAglomerateSlab[$.id(^form:id.int(0))]]

<div id="">
<h1>Подоконник из $name ${detail_windowsill_$item.aglomerate_windowsill_name}. Образец.</h1>
</div>
...
<a href="/services/makestone/windowsill/$item/img/big/${detail_windowsill_$item.aglomerate_windowsill_img}.webp" target="_blank" title="Подоконник из $name $detail_windowsill_$item.aglomerate_windowsill_name для кунхи"><img src="/services/makestone/windowsill/$item/img/list/${detail_windowsill_$item.aglomerate_windowsill_img}.webp" alt="Подоконник из $name $detail_windowsill_$item.aglomerate_windowsill_name для кунхи" /></a>
...
<h3>Другие изделия из $name $detail_tabletop_$item.aglomerate_windowsill_name</h3>
...
<li><a href="/services/makestone/table/$item/detail/id/$table_$item.id/" title="Стол из $name $table_$item.aglomerate_table_name">Cтол из $name $table_$item.aglomerate_table_name</a></li>
</ul>
