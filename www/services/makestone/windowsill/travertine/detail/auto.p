...
@title[]
$title[^db::getTravertineWindowsill[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Подоконник из травертина $title.travertine_windowsill_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getTravertineWindowsill[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Подоконник из $name $navi_center_id.travertine_windowsill_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_windowsill_$item[^db::getTravertineWindowsill[$.id(^form:id.int(0))]]
$tabletop_$item[^db::getTravertineTableTop[$.id(^form:id.int(0))]]
$step_$item[^db::getTravertineStep[$.id(^form:id.int(0))]]
$table_$item[^db::getTravertineTable[$.id(^form:id.int(0))]]
$slab_$item[^db::getTravertineSlab[$.id(^form:id.int(0))]]

<div id="">
<h1>Подоконник из $name ${detail_windowsill_$item.travertine_windowsill_name}. Образец.</h1>
</div>
...
<a href="/services/makestone/windowsill/$item/img/big/${detail_windowsill_$item.travertine_windowsill_img}.webp" target="_blank" title="Подоконник из $name $detail_windowsill_$item.travertine_windowsill_name для кунхи"><img src="/services/makestone/windowsill/$item/img/list/${detail_windowsill_$item.travertine_windowsill_img}.webp" alt="Подоконник из $name $detail_windowsill_$item.travertine_windowsill_name для кунхи" /></a>
...
<h3>Другие изделия из $name $detail_tabletop_$item.travertine_windowsill_name</h3>
...
<li><a href="/services/makestone/step/$item/detail/id/$step_$item.id/" title="Ступень из $name $step_$item.travertine_step_name">Ступень из $name $step_$item.travertine_step_name</a></li>
