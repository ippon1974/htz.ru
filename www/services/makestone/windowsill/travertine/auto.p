...
$result[Подоконники из травертина]
} 
#@title[]
...
^opage.print[
	$.target_url[/services/makestone/windowsill/travertine/]
	$.nav_count(25)
	$.mode[html]
]
$travertine[^MAIN:pSQL.table{
	SELECT
		travertine_windowsill_id AS id,
		travertine_slab_id,
		travertine_windowsill_type_id,
		travertine_windowsill_name,
		travertine_windowsill_price,
		travertine_windowsill_item,
		travertine_windowsill_desc,
		travertine_windowsill_img,
		is_published
	FROM
		v_windowsill_travertine
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND travertine_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
<ul class="listing">
	^travertine.menu{
	       ^printList_cell[travertine;травертина]
	}
</ul>
}
#end @printList[opage]
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.travertine_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.travertine_windowsill_img}.webp" alt="Подоконник из $name ${$item.travertine_windowsill_name}" /></a></dt>
