...
@title[]
$title[^db::getMarbleWindowsill[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Подоконник из мрамора $title.marble_windowsill_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getMarbleWindowsill[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Подоконник из $name $navi_center_id.marble_windowsill_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_windowsill_$item[^db::getMarbleWindowsill[$.id(^form:id.int(0))]]
...
$slab_$item[^db::getMarbleSlab[$.id(^form:id.int(0))]]

<div id="">
<h1>Подоконник из $name ${detail_windowsill_$item.marble_windowsill_name}. Образец.</h1>
</div>
...
<a href="/services/makestone/windowsill/$item/img/big/${detail_windowsill_$item.marble_windowsill_img}.webp" target="_blank" title="Подоконник из $name $detail_windowsill_$item.marble_windowsill_name для кунхи"><img src="/services/makestone/windowsill/$item/img/list/${detail_windowsill_$item.marble_windowsill_img}.webp" alt="Подоконник из $name $detail_windowsill_$item.marble_windowsill_name для кунхи" /></a>
...
<h3>Другие изделия из $name $detail_tabletop_$item.marble_windowsill_name</h3>
...
<a href="/services/makestone/catalogstone/$item/detail/id/$slab_$item.id/" title="Слеб из $name $slab_$item.m_name"><img src="/services/makestone/catalogstone/$item/img/list/${slab_$item.m_img}.webp" alt="Слеб из $name $slab_$item.m_name" /></a>
