...
^opage.print[
	$.target_url[/services/makestone/windowsill/marble/]
	$.nav_count(50)
	$.mode[html]
]
$marble[^MAIN:pSQL.table{
	SELECT
		marble_windowsill_id AS id,
		m_slab_id,
		marble_windowsill_type_id,
		marble_windowsill_name,
		marble_windowsill_price,
		marble_windowsill_item,
		marble_windowsill_desc,
		marble_windowsill_img,
		is_published
	FROM
		v_windowsill_marble
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND marble_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.marble_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.marble_windowsill_img}.webp" alt="Подоконник из $name ${$item.marble_windowsill_name}" /></a></dt>
