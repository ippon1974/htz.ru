...
^opage.print[
	$.target_url[/services/makestone/windowsill/quartz/]
	$.nav_count(50)
	$.mode[html]
]
$quartz[^MAIN:pSQL.table{
	SELECT
		quartz_windowsill_id AS id,
		quartz_slab_id,
		quartz_windowsill_type_id,
		quartz_windowsill_name,
		quartz_windowsill_price,
		quartz_windowsill_item,
		quartz_windowsill_desc,
		quartz_windowsill_img,
		is_published
	FROM
		v_windowsill_quartz
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND quartz_windowsill_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	<li>
	<dl>
	<dt><a href="/services/makestone/windowsill/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.quartz_windowsill_name}"><img src="/services/makestone/windowsill/$item/img/list/${$item.quartz_windowsill_img}.webp" alt="Подоконник из $name ${$item.quartz_windowsill_name}" /></a></dt>
