...
$amethyst[^MAIN:pSQL.table{
	SELECT
		amethyst_table_id AS id,
		amethyst_slab_id,
		amethyst_table_type_id,
		amethyst_table_name,
		amethyst_table_price,
		amethyst_table_item,
		amethyst_table_desc,
		amethyst_table_img,
		is_published
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND amethyst_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.amethyst_table_name}">Стол из $name ${$item.amethyst_table_name}</a></dd>
	</dl>
	</li>
]
