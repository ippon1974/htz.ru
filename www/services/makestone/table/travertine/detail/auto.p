...
@title[]
$title[^db::getTravertineTable[$.id(^form:id.int(0))]]
^if(def $title){ 
$result[Стол из травертина $title.travertine_table_name]
}{ 
$result[]
} 
#@title[]
...
@navi_center_id[name]
$navi_center_id[^db::getTravertineTable[$.id(^form:id.int(0))]]
$result[<span style="background: #FFFFCC^;">Стол из $name $navi_center_id.travertine_table_name</span>]
#@navi_center_id[]

@tabletop_detail[item;name]
^hMakeStone[]
$detail_step_$item[^db::getTravertineTable[$.id(^form:id.int(0))]
...
<div id="">
<h1>Стол из $name ${detail_step_$item.travertine_table_name}. Образец.</h1>
</div>
...
<a href="/services/makestone/table/$item/img/big/${detail_step_$item.travertine_table_img}.webp" target="_blank" title="Стол из $name $detail_step_$item.travertine_table_name"><img src="/services/makestone/table/$item/img/list/${detail_step_$item.travertine_table_img}.webp" alt="Ступень из $name $detail_step_$item.travertine_table_name" /></a>
...
<h3>Другие изделия из $name $detail_step_$item.travertine_table_name</h3>
