...
^opage.print[
	$.target_url[/services/makestone/table/travertine/]
	$.nav_count(25)
	$.mode[html]
]
$travertine[^MAIN:pSQL.table{
	SELECT
		travertine_table_id AS id,
		travertine_slab_id,
		travertine_table_type_id,
		travertine_table_name,
		travertine_table_price,
		travertine_table_item,
		travertine_table_desc,
		travertine_table_img,
		is_published
	FROM
		v_table_travertine
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND travertine_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.travertine_table_name}">Стол из $name ${$item.travertine_table_name}</a></dd>
	</dl>
	</li>
]
