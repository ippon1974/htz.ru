...
^opage.print[
	$.target_url[/services/makestone/table/labradorite/]
	$.nav_count(25)
	$.mode[html]
]
$labradorite[^MAIN:pSQL.table{
	SELECT
		labradorite_table_id AS id,
		labradorite_slab_id,
		labradorite_table_type_id,
		labradorite_table_name,
		labradorite_table_price,
		labradorite_table_item,
		labradorite_table_desc,
		labradorite_table_img,
		is_published
	FROM
		v_table_labradorite
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND labradorite_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.labradorite_table_name}">Стол из $name ${$item.labradorite_table_name}</a></dd>
	</dl>
	</li>
]
