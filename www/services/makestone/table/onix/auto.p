...
^opage.print[
	$.target_url[/services/makestone/table/onix/]
	$.nav_count(50)
	$.mode[html]
]
$onix[^MAIN:pSQL.table{
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND onix_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.onix_table_name}">Стол из $name ${$item.onix_table_name}</a></dd>
	</dl>
	</li>
]
