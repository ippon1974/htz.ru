...
^opage.print[
	$.target_url[/services/makestone/table/aglomerate/]
	$.nav_count(25)
	$.mode[html]
]
$aglomerate[^MAIN:pSQL.table{
	SELECT
		aglomerate_table_id AS id,
		aglomerate_slab_id,
		aglomerate_table_type_id,
		aglomerate_table_name,
		aglomerate_table_price,
		aglomerate_table_item,
		aglomerate_table_desc,
		aglomerate_table_img,
		is_published
	FROM
		v_table_aglomerate
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND aglomerate_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.aglomerate_table_name}">Стол из $name ${$item.aglomerate_table_name}</a></dd>
	</dl>
	</li>
]
