...
^opage.print[
	$.target_url[/services/makestone/table/quartz/]
	$.nav_count(25)
	$.mode[html]
]
$quartz[^MAIN:pSQL.table{
	SELECT
		quartz_table_id AS id,
		quartz_slab_id,
		quartz_table_type_id,
		quartz_table_name,
		quartz_table_price,
		quartz_table_item,
		quartz_table_desc,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND quartz_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.quartz_table_name}">Стол из $name ${$item.quartz_table_name}</a></dd>
	</dl>
	</li>
]
