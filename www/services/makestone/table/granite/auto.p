...
^opage.print[
	$.target_url[/services/makestone/table/granite/]
	$.nav_count(50)
	$.mode[html]
]
$granite[^MAIN:pSQL.table{
	SELECT
		granite_table_id AS id,
		g_slab_id,
		granite_table_type_id,
		granite_table_name,
		granite_table_price,
		granite_table_item,
		granite_table_desc,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND granite_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.granite_table_name}">Стол из $name ${$item.granite_table_name}</a></dd>
	</dl>
	</li>
]
