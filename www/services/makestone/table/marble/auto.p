...
^opage.print[
	$.target_url[/services/makestone/table/marble/]
	$.nav_count(50)
	$.mode[html]
]
$marble[^MAIN:pSQL.table{
	SELECT
		marble_table_id AS id,
		m_slab_id,
		marble_table_type_id,
		marble_table_name,
		marble_table_price,
		marble_table_item,
		marble_table_desc,
	...
	WHERE
		is_published = 1
		^if(^hParam.id.int(0)){
		AND marble_table_id = ^hParam.id.int(0)
		}
	ORDER BY
		id ASC
}[
...
$result[
	...
	<dd class="delta"><a href="/services/makestone/table/$item/detail/id/${$item.id}/" title="Подоконник из $name ${$item.marble_table_name}">Стол из $name ${$item.marble_table_name}</a></dd>
	</dl>
	</li>
]
