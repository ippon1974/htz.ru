...
@description[]
  $description[^getProducts[$.id(^form:id.int(0))]]
       $result[Запасные части для трактора МТЗ-80]
#@description[]

@keywords[]
$keywords[^getProducts[$.id(^form:id.int(0))]]
       $result[Запчасти МТЗ-80, Запчасти для трактора МТЗ-80]
#@keywords[]
 ...
 ^if(^form:id.int(0)){
	...
	$cat_cat_cat[^getCategories[$.id($cat_cat.parent)]]
	 ...
	 <div style="margin: 0 0 1em 0^;font-size: 1.2em^;">
      ...
      <a href="/catalog/spare/mtz/80/products/id/$cat.id/" title="$cat.name">$cat.name</a>
     </div>
